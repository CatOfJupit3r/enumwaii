...
/**
 * Adapts an enumwaii declaration to a Valibot schema for integrations that require Valibot's schema type.
 *
 ...
 * Use this optional subpath when a Valibot consumer cannot accept the declaration's native Standard Schema implementation;
 * parsing an invalid value follows Valibot's normal issue/error behavior.
 * The `valibot` peer dependency must be installed by the application.
 *
 * @param enumeration Declaration whose membership should be validated.
 * @returns A Valibot custom schema typed as the declaration's branded member union.
 *
 * @example
 * ```ts
